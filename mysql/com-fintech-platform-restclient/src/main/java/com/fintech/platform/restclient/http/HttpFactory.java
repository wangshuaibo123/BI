...
import com.fintech.platform.cfg.RestConfig;
import com.fintech.platform.core.message.MediaType;
import com.fintech.platform.tools.common.CommonUtil;
...
//		httpClient=HttpClients.custom().setConnectionManager(connManager).setDefaultRequestConfig(config).setRetryHandler(myRetryHandler).build();
	...
	private  <T> void  processHttpRequest(String appId, HttpUriRequest request,String contentType, T t){
		...
		if(appId!=null&&!appId.equals("")){
			request.setHeader(new BasicHeader("Authorization", encrypt(appId, RestClientConfig.getAppPwd(appId))));
		}
