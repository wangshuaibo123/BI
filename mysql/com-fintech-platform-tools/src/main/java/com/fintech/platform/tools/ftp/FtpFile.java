package com.fintech.platform.tools.ftp;
...
public class FtpFile {
    ...
    private void cdOrMkMoreDirPath(FTPClient ftp,String ftpPath){
    	if(ftpPath.indexOf("/")>-1){
    		...
    		cdOrMkDir(ftp,ftpPath);
    	}
    }
    private void cdOrMkDir(FTPClient ftp,String path){
		try {
			try {
                ftp.chdir(path);
            } catch (Exception ex) {
    ...
    public boolean putInputStreamToFtp(InputStream inputStream,String ftpPath, String uploadFileName) {
        boolean retftp = false;
        try {
        	FTPClient ftp = login();
            if(passive){
            ...
            ftp.put(inputStream, uploadFileName);
            ftp.quit();
            retftp = true;
        } catch (Exception e) {
            this.logger.error(e.getMessage());
    ...
    public static void main(String[] args) { 
    	FtpFile ftp = new FtpFile("192.168.1.101", "qhcjr", "BWPsoft2016");
//    	ftp.putFile("H:\\DbVisualizer.rar", "/home/jyapp/jyintofile"); 
    	//ftp.renameFile("DbVisualizer.rar","1111111111111111.rar");
    	ftp.ftpPath="upload_file/aaa/bbb/ccc/ddd";
    	ftp.putFileToFtp("Q:/a.jpg", "aaa.jpg");
    }
