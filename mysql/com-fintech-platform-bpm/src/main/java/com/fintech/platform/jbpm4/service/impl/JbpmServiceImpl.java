 ...
 */
@Service("com.fintech.platform.jbpm4.service.impl.JbpmServiceImpl")
...
public class JbpmServiceImpl implements IJbpmService,Serializable {
	...
	@Autowired
	@Qualifier("com.fintech.platform.jbpm4.service.impl.ModifyProInfoServiceImpl")
	...
	@Autowired
	@Qualifier("com.fintech.platform.jbpm4.service.impl.ConsignTaskServiceImpl")
	private ConsignTaskService consignBiz;
						...
						public void run() {
							try {
								...
								if(defMap != null){
									...
									String proDefVersion = String.valueOf(defMap.get("VERSION"));
									//获取表单 配置 及选人规则的配置信息
	...
	@SuppressWarnings("all")
	public List queryActiveProcessInfoListByPaging(Map<String, Object> params) throws Exception {
		if(params != null){
			JbpmDTO dto  = (JbpmDTO) params.get("dto");
			 //是否管理监控模块
			if(dto != null  && "1".equals(dto.getIsMonitor())){
		...
		try {
			...
			list = mapper.queryActiveProInfoByMySqlByPaging(params);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	/*
	 * (non-Javadoc)
	 * @see com.fintech.platform.jbpm4.service.IJbpmService#queryEndProInfoList(java.util.Map)
	 */
	...
	public String getAllTurenDirectory(String processInstanceId,String currentActiveName) {
	...
	/*
	 * (non-Javadoc)
	 * @see com.fintech.platform.jbpm4.service.IJbpmService#getIncomingTransitions(java.lang.String, java.lang.String)
	 */
...
/*
 * (non-Javadoc)
 * @see com.fintech.platform.jbpm4.service.IJbpmService#obtainJbpm4FormByProInsId(java.lang.String)
 */
	...
	/*
	 * (non-Javadoc)
	 * @see com.fintech.platform.jbpm4.service.IJbpmService#viewWorkflowHisLog(java.util.Map)
	 */
	public List<Map<String, Object>> viewWorkflowHisLog(
		...
		if(resultList != null && resultList.size() >0){
			...
			resultList = consignBiz.makeupConsignInfo(resultList);
			for(int i = resultList.size() -1 ; i >= 0 ;i --){
				...
				String consignDesc = (String) tem.get("CONSIGN_DESC");
				if(StringUtils.isNotEmpty(consignDesc)){
					
					tem.put("LOG_DESC", consignDesc+sb.toString());
				}else{
    			...
    			if(null!=roleMappingList && roleMappingList.size()>0){
    				...
    				if("PROCESS_TASK".equals(dto.getProcessState())){
    					roleMappingSql.append(" and (assignee_ in")
    								  .append(" (select CONCAT(ru.target_id) from sys_role_user ru, sys_role r ")
    					...
    					roleMappingSql.append(" and (assignee_ in ( select CONCAT(ru.target_id) from sys_role_user ru, sys_role r where ru.role_id = r.id ")
						  			  .append("  and r.role_code in (").append(mappingRoles.toString()).append("))")
						  			  .append(" or assignee_='").append(userId).append("')");
    				}else if("END_TASK".equals(dto.getProcessState())){
    					roleMappingSql.append(" and exists ( select CONCAT(ru.target_id) from sys_role_user ru, sys_role r where ru.role_id = r.id ")
    					.append("  and exists (select ht.execution_ from jbpm4_hist_task ht where ht.assignee_ = CONCAT(ru.target_id) and ht.execution_ = t1.pro_instance_id) and r.role_code in (")
    					.append(mappingRoles.toString()).append(" ) )") ;
    					
    				}else if("END_HIST_TASK".equals(dto.getProcessState())){//by xyz
    					roleMappingSql.append(" and exists ( select CONCAT(ru.target_id) from sys_role_user ru, sys_role r where ru.role_id = r.id ")
    					.append("  and exists (select ht.execution_ from jbpm4_hist_task_old ht where ht.assignee_ = CONCAT(ru.target_id) and ht.execution_ = t1.pro_instance_id) and r.role_code in (")
    					.append(mappingRoles.toString()).append(" ) )") ;
