...
public class BestWorkFlowProviderController extends BaseController{
    ...
    public DataMsg findWorkFlowInfo(JbpmDTO dto, @ModelAttribute DataMsg dataMsg) throws Exception {
        ...
        if (dataMsg == null) dataMsg = new DataMsg();
 		...
 		}else if("MY_COMPLETED_TASK".equals(dto.getProcessState())){//已办结的历史任务 查询
 			dataMsg = this.queryMyCompletedTaskByMySql(dto,dataMsg);
 		}
    ...
    private DataMsg queryMyCompletedTaskByMySql(JbpmDTO dto, DataMsg dataMsg) throws Exception {
    	//使用分页查询
    ...
    */
	...
	private DataMsg queryActiveProcessInfoByMySql(JbpmDTO dto, DataMsg dataMsg) throws Exception {
		...
		List resultList = jbpmServcie.queryActiveProcessInfoListByPaging(paramsPT.getSearchParams());
