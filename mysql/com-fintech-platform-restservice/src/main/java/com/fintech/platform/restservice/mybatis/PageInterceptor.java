...
public class PageInterceptor implements Interceptor {
    ...
    private static String defaultDialect = "mysql";//"oracle"; // 数据库类型(默认为mysql)
    private static String defaultPageSqlId = ".*ByPaging$"; // 需要拦截的ID(正则匹配)
    private static String dialect = "mysql";//"oracle"; // 数据库类型(默认为mysql)
        ...
        if (mappedStatement.getId().matches(pageSqlId)) {
            ...
            if (parameterObject == null) {
                ...
                if("oracle".equals(dialect)){
                    ...
                    endrow=String.valueOf(page.getPageSize());
                    boundSql.setAdditionalParameter("beginRown", new BigDecimal(beginrow));
                    boundSql.setAdditionalParameter("endRown", new BigDecimal(endrow));
                }
              ...
              //单独执行业务查询sql
                if("N".equalsIgnoreCase(page.getbizTotalFlagFlag())){
                    logger.debug("========only execute  biz query sql====");
                    return invocation.proceed();
                }
                
                //单独计算总数
                if("Y".equalsIgnoreCase(page.getbizTotalFlagFlag())){
                    ...
                    boundSql.setAdditionalParameter("beginRown", new BigDecimal(0));
                    boundSql.setAdditionalParameter("endRown", new BigDecimal(0));
