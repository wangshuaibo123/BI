 ...
 *
 */
@Component("com.fintech.modules.platform.sysasynjob.service.ExcSMSAsynJobTask")
public class ExcSMSAsynJobTask implements Serializable,Job{
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LoggerFactory.getLogger(ExcSMSAsynJobTask.class);
	//线程池初始化
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		if(!isNext){
			logger.info("----------进入方法ExcSMSAsynJobTask execute---------------");
			return ;
		}
		logger.info("----------ExcSMSAsynJobTask execute start---------------");
		isNext = false;
		try {
			...
			asynParam.put("jobState", "2");// 发送短信异步接口 待调用
			asynParam.put("runCun", "1");// 异常任务最多尝试2次
			...
			if(asynDtoList != null && asynDtoList.size() > 0){
				...
				for(SysAsynJobDTO dto :asynDtoList){
					...
					if(PlatformThreadTool.threadMap.size() > 1000){
						//如果队列过大 则不在放入
						logger.error("====执行 ExcSMSAsynJobTask 队列过大，等待下次异步处理====");
						break;
					}
			...
			logger.error(" 执行 ExcSMSAsynJobTask 异常.........错误明细："+ex.getMessage());
		}finally{
			logger.info("----------ExcSMSAsynJobTask execute end---------------");
			isNext= true;
		}
	}
