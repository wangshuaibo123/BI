package com.fintech.modules.platform.bizauth.vmuservmorgmap.service;
 ...
 * @author
 */
@Service("com.fintech.modules.platform.bizauth.vmuservmorgmap.service.VmuserVmorgMapService")
public class VmuserVmorgMapService implements Serializable {
	private static final Logger logger = LoggerFactory.getLogger(VmuserVmorgMapService.class);
	...
	public void validateVmuserVmorgMap(String userId,String orgId,String orgType) throws Exception{
		...
		if(StringUtils.isNotEmpty(userId)){
			...
			if(dataList != null && dataList.size() >0) throw new Exception("用户ID:"+userId+"对应的组织机构代码和类型已经存在,不能再次添加,请重新选择！！！");
		}
	...
	public Long insertVmuserVmorgMap(VmuserVmorgMapDTO dto,String type) throws Exception {
    	...
    	//一、先判断是否是HR的叶子节点
    	if("HR".equals(vmtreeInfoDTO.getSourceType())){
    		     //如果是HR机构，并且是最低层机构endflag为1。 则新增  数据权限表  User_Id, Owner_Id
	    		if("1".equals(vmtreeInfoDTO.getEndFlag())){
	    			 //查询本HR下面管理的人员
	    			if(vmOrgUserList.size()> 0){
	    			...
	    			if(vmtreedtoHRlist.size()>=1){
	    				for(VmtreeInfoDTO vmtreedto:vmtreedtoHRlist){
	    					//找下级是HR叶子节点
	    					if(!"1".equals(vmtreedto.getEndFlag())) continue;
	 ...
	 * 获取树形orgids
	 */
    public String findVmuserVmorgMapTreeByOrgId(Map<String, Object> paramMap)throws Exception{
    	String retStr = "";
    	try {
			if(paramMap.get("orgId")==null || "".equals(paramMap.get("orgId").toString()) || paramMap.get("orgType")==null)return null;
			Map<String,Object> map=dao.findVmuserVmorgMapTreeByOrgId(paramMap);
			retStr =  map.get("ORGIDS")==null?"":map.get("ORGIDS").toString();
		} catch (Exception e) {
			logger.error("findVmuserVmorgMapTreeByOrgId===error"+e.getMessage(),e);
		}
    	return retStr;
    }
}
