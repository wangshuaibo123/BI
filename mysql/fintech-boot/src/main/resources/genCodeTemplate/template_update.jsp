		...
		<table id="updateTableId" class="formTableSwap" border="0" align="center" cellpadding="0" cellspacing="1">
			...
			<tr>
				...
				<td <#if ( ((dataInfoList ? size) -1) == d_inde ) && (dataInfoList ? size)%indexNum != 0 >colspan="${ (indexNum- ((d_inde+1) %indexNum))*2 +1}"</#if>>
					<input type="text" class="text" id="${dataInfo.DTO_COLUMN_NAME?replace(".","")}" name="${dataInfo.DTO_COLUMN_NAME?replace("dto.","")}" notNull="true" maxLength="${(dataInfo.DATA_LENGTH)}" value="${'$'}{${dataInfo.DTO_COLUMN_NAME}}" checkChName="${column_name}" />
				</td>
