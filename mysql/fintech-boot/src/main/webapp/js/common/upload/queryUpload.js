...
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/upload/prepareExecute/toAdd',
		'width':800,
		'height':500,
		'title':'新增',
		'buttons':[
...
function doAddFrom(){
	...
	var url=contextRootPath+'/upload/insertUpload';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function toUpdateData(){
	...
	var dialogStruct={
			'display':contextRootPath+'/upload/prepareExecute/toUpdate?id='+v_ids,
			'width':800,
			'height':500,
			'title':'修改',
			'buttons':[
...
function doUpdateFrom(){
	...
	var url=contextRootPath+'/upload/updateUpload';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function deleteData(){
	...
	jyDialog({"type":"question"}).confirm("您确认要删除选择的数据吗？",function(){
		$.ajax({
            type:"POST",
            dataType:"json",
            url:contextRootPath+"/upload/deleteUpload?ids="+v_ids,
...
function viewData(vId){
	var dialogStruct={
			'display':contextRootPath+'/upload/prepareExecute/toView?id='+vId,
			'width':800,
			'height':500,
			'title':'查看明细',
			'buttons':[
