...
public class ForceLogoutFilter extends AccessControlFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue)  {
        ...
        if(!StringUtils.isEmpty(tag)){//数据库有此值
        	...
        	//拒绝登录
        	if(tagBoolean){
        		String notRefuseLoginRole = sysConfigService.realGainValueByCode("notRefuseLoginRole");//可以登录的角色code
        		
        		if(notRefuseLoginRole!=null && !"".equals(notRefuseLoginRole)){
        			List<Map<String, Object>> roleList = sysRoleAPI.getRoleByUserId(currentUser.getUserId().toString());
            		if(roleList!=null && roleList.size()>0){
            			...
            			for(int i=0;i<roleList.size();i++){
            				...
            				//当前角色不拒绝登录
            				if(!StringUtils.isEmpty(notRefuseLoginRole) && !StringUtils.isEmpty(roleMap.get("roleCode"))
            						&& notRefuseLoginRole.equals(roleMap.get("roleCode"))){
            					return true;
            				}
            			}
            		}
    ...
    private boolean checkLongLinkAccess(ServletRequest request){
        ...
        //http://app.test.com:8180/com-jy-platform-portal/sysMessagecenter/doCountMessage
