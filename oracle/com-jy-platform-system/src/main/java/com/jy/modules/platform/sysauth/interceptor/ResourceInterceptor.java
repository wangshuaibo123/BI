package com.jy.modules.platform.sysauth.interceptor;
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(handler instanceof HandlerMethod){
			...
			try{
				/**
				  * 非法字符配置文件
				  */
				illegalProperties = ResourceBundle.getBundle("illegal_character");
		...
		if(handler instanceof HandlerMethod){
			...
			if(hand.getMethod().getDeclaringClass().getSimpleName().endsWith("Controller")){//判断请求的都是Controller不是rest请求
				...
				SysResourceService service=(SysResourceService) context.getBean("com.jy.modules.platform.sysauth.service.SysResourceService");
				List<SysResourceRoleDTO> allUrl = service.getSysResourceAllUrl("");
				SessionAPI sessionAPI =(SessionAPI) context.getBean("sessionAPI");
				SysRoleService roleService= (SysRoleService) context.getBean("com.jy.modules.platform.sysauth.service.SysRoleService");
