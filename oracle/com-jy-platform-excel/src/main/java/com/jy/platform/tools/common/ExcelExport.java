...
//	    moneyStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER);
    ...
    public SXSSFWorkbook exportLargeExcel(String sheetName, String[] headers, String[] types,List dataset, String[] propertiesName,
            List attributesOfDateType, Map comboxColumn, SXSSFWorkbook workbook, int pageNum, String isTime, String[] excelStyles) throws Exception {
        try {
...
//			                            cell.setCellValue(format.format(gc.getTime()));
			                 ...
			                 else if(types[j]!=null && "money".equals(types[j])){
			                	 cell.setCellType(XSSFCell.CELL_TYPE_NUMERIC);
			                	 try{
			                		 cell.setCellValue(Double.parseDouble(objValue.toString()));
			                	 }
			                	 catch(Exception e){logger.error("parse double error: ",e);}
			                 }
			                 else {
			                     cell.setCellType(XSSFCell.CELL_TYPE_STRING);
			                     cell.setCellValue(objValue.toString());
			            ...
			            //设置自定义样式
			            if(excelStyles[j]!=null && !"".equals(excelStyles[j])){
			                CellStyle customStyle = workbook.createCellStyle();
			                customStyle.setDataFormat(workbook.createDataFormat().getFormat(excelStyles[j]));
