...
public class ProcessThreadMysql implements Runnable {
	
	private static Logger logger = LoggerFactory.getLogger(ProcessThreadMysql.class);
	...
	public ProcessThreadMysql(String threadNo) {
	...
	private void process(ThreadConf threadConf) {
		...
		try {
			...
			while (rs.next()) {
				...
				pstmt.setObject(paramIdx, rs.getObject("ID"));
				pstmt.addBatch();
