...
import com.jy.platform.jbpm4.assign.GetAllTaskNameCmd;
import com.jy.platform.jbpm4.assign.GetHistTaskNameCmd;
import com.jy.platform.jbpm4.assign.JumpTaskCommand;
import com.jy.platform.jbpm4.assign.MainProSingleWayCmd;
import com.jy.platform.jbpm4.assign.RegectTaskSingleBackWayCmd;
import com.jy.platform.jbpm4.assign.SubProSingleWayCmd;
/**
...
public class MyJBPMTool {
	...
	@Autowired
	@Qualifier("com.jy.platform.jbpm4.assign.RegectTaskSingleBackWayCmd")
	private RegectTaskSingleBackWayCmd backWayCmd;
	
	@Autowired
	@Qualifier("com.jy.platform.jbpm4.assign.MainProSingleWayCmd")
	private MainProSingleWayCmd mainWayCmd;
	
	@Autowired
	@Qualifier("com.jy.platform.jbpm4.assign.SubProSingleWayCmd")
	 ...
	 * @param variables xyz
	 */
	public void completeJumpTask(String taskId, String turnDir, String targetTaskName, Map<String, Object> variables) {
		
	    JumpTaskCommand compositeCmd = new JumpTaskCommand();
	    compositeCmd.setVariables(variables);
	    compositeCmd.setTaskId(taskId);
	    compositeCmd.setOutcome(turnDir);
	    compositeCmd.setTargetName(targetTaskName);
	...
	public List<String> getHistTaskName(String processInstanceId){
		GetHistTaskNameCmd getHistTaskNameCmd=new GetHistTaskNameCmd(processInstanceId);
		return processEngine.execute(getHistTaskNameCmd);
	}
	/**
	 * 查询所有数据
	 * @param processInsId
	 * @return
	 */
	public List<String> getAllTaskName(String processInsId) {
		GetAllTaskNameCmd getAllTaskNameCmd=new GetAllTaskNameCmd(processInsId);
		return processEngine.execute(getAllTaskNameCmd);
	}
	/**
	 * 根据当前节点和目的节点信息创建动态路由
	 ...
	 * @return
	 */
	public  TransitionImpl  createTransition(ActivityImpl sourceActivity,ActivityImpl currentActivity )throws Exception {
	 ...
	 * @param sourceActivity
	 */
	public void updateExecution(TaskImpl currentTask,  ActivityImpl sourceActivity) throws Exception {
		
		ExecutionImpl exec = currentTask.getProcessInstance();
		exec.execute(sourceActivity);
	}
}
